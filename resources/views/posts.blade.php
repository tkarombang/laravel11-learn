<x-layout>
  ...
  <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-8 lg:px-0">
    ...
    <div>
      By
      <a href="/authors/{{ $post->author->username }}" class="text-base text-indigo-800 hover:underline font-semibold">{{ $post->author->name }}</a>
      In
      <a href="/categories/{{ $post->category->nama_category }}" class="text-base text-indigo-800 hover:underline font-extrabold">{{ $post->category->nama_category }}</a> |
    ...
    <a href="/posts/{{ $post['slug'] }}" class="font-medium text-blue-500 hover:underline">Read more &raquo;</a>
      ...
      <article class="p-6 bg-gray-200 rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        <div class="flex justify-between items-center mb-5 text-gray-500">
          <a href="/categories/{{ $post->category->nama_category }}">
            <span class="bg-{{ $post->category->color }}-100 text-primary-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-primary-200 dark:text-primary-800">
              {{ $post->category->nama_category }}
            </span>
          </a>
